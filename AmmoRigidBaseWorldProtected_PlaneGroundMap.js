...
class App {
                            ...
                            for(let s in moveForm_parameters){
                                ...
                                if(s==='loadposx'){
                                   // var key=CryptoJS.enc.Utf8.parse('-333');//원본밸류의 key값만듬.
                                    ...
                                    moveForm_parameters['loadposx']='18';
                                }else if(s==='loadposy'){
                                    moveForm_parameters['loadposy']='333';
                                    ...
                                    moveForm_parameters['loadposz']='-1171';
                            ...
                            for(let s in moveForm_parameters){
                                ...
                                if(s==='loadposx'){
                                   // var key=CryptoJS.enc.Utf8.parse('-333');//원본밸류의 key값만듬.
                                    ...
                                    moveForm_parameters['loadposy']='-296';
                                    //var key=CryptoJS.enc.Utf8.parse('7');//원본밸류의 key값만듬.
                                    //console.log("각문자열에 대한 경우의수별 랜덤한 key값!:",key);
                                    //var base64=CryptoJS.enc.Base64.stringify(key);
                                    //parameter.value=base64;
                                    ...
                                    moveForm_parameters['loadposz']='-1308';
                                      //var key=CryptoJS.enc.Utf8.parse('2220');//원본밸류의 key값만듬.
                                      //console.log("각문자열에 대한 경우의수별 랜덤한 key값!:",key);
                                      //var base64=CryptoJS.enc.Base64.stringify(key);
                                      //parameter.value=base64;
    ...
    createAmmo(Ammo=this.ammoClone,){
        ...
        this.createGLTFGroup('https://sinjaesung.github.io/3DASSET/terrain/evil/planeGroundMap.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{//convexhull로해야만함!!!
            'shapeType':'convexTriangle'
        }); 
        this.createGLTFGroup('https://sinjaesung.github.io/3DASSET/terrain/evil/planeGroundMap_entrance.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{//convexhull로해야만함!!!
            'shapeType':'convexTriangle'
        }); 
        this.createGLTFGroup('https://sinjaesung.github.io/3DASSET/terrain/evil/planeGroundMap_nextMoveRoot.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{//convexhull로해야만함!!!
            'shapeType':'convexTriangle'
        }); 
        //포탈!!(맵간 이동) 별도 파라미터 넣지않으면 이동시 맵의 초기설정위치에서 로드,pos넣을시 그 post에 관련된 위치에서 카메라랑,캐릭터랑 해서 로드
        this.NextPORTAL('https://sinjaesung.github.io/3DASSET/terrain/evil/planeGroundMap_Nextfortal.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{
            'shapeType':'convexTriangle'
        });
        this.StartPORTAL('https://sinjaesung.github.io/3DASSET/terrain/evil/planeGroundMap_Startfortal.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{
            'shapeType':'convexTriangle'
        });  
        // 몬스터
       ...
       for(let e=0; e<10; e++){//humanBad created Types (메카닉:전투기계들,바이오닉,사이보그)
        ...
        for(let e=0; e<15; e++){//humanBad created Types (메카닉:전투기계들,바이오닉,사이보그)
            let unit=e%2==0?-1:1;
            let x=Math.random()*80*unit; //-500~500범위가능
            let y=Math.random()*3+70;//항상양수
            let z=Math.random()*80*unit
        ...
        added_loadmodel_bg['src']="https://sinjaesung.github.io/3DASSET/garden_nook_4k.hdr";
        added_loadmodel_bg['isloaded']=false;
        this.loadmodels.push(added_loadmodel_bg);

          new RGBELoader().load("https://sinjaesung.github.io/3DASSET/garden_nook_4k.hdr",(texture)=>{
