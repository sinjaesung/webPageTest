...
class App {
                ...
                if(this.character1.userData.hp <= 0){
                    ...
                    if(this.NextEntrance){
                            ...
                            for(let s in moveForm_parameters){
                                ...
                                if(s==='loadposx'){
                                   // var key=CryptoJS.enc.Utf8.parse('-333');//원본밸류의 key값만듬.
                                    ...
                                    moveForm_parameters['loadposx']='-2';
                                }else if(s==='loadposy'){
                                    moveForm_parameters['loadposy']='24';
                                    ...
                                    moveForm_parameters['loadposz']='-592';
                        ...
                        window.setTimeout(()=>{
                            ...
                            for(let s in moveForm_parameters){
                                ...
                                if(s==='loadposx'){
                                   // var key=CryptoJS.enc.Utf8.parse('-333');//원본밸류의 key값만듬.
                                    ...
                                    moveForm_parameters['loadposx']='-91';
                                }else if(s==='loadposy'){
                                    moveForm_parameters['loadposy']='-125';
                                    //var key=CryptoJS.enc.Utf8.parse('7');//원본밸류의 key값만듬.
                                    //console.log("각문자열에 대한 경우의수별 랜덤한 key값!:",key);
                                    //var base64=CryptoJS.enc.Base64.stringify(key);
                                    //parameter.value=base64;
                                    ...
                                    moveForm_parameters['loadposz']='782';
                                      //var key=CryptoJS.enc.Utf8.parse('2220');//원본밸류의 key값만듬.
                                      //console.log("각문자열에 대한 경우의수별 랜덤한 key값!:",key);
                                      //var base64=CryptoJS.enc.Base64.stringify(key);
                                      //parameter.value=base64;
                            ...
                            //mapMoveForm.submit();
                            localStorage.setItem("characterdatabase",JSON.stringify(moveForm_parameters));
                            location.href='AmmoRigidBaseMetaWorldEnemyStart.html';
                        },200);
                    } 
                    //맵별로 다르게 할부분 endsss`
                }
    ...
    createAmmo(Ammo=this.ammoClone,){
        ...
        //지형지물관련
        this.createGLTFGroup('https://sinjaesung.github.io/3DASSET/terrain/evil/evilFactoryBase.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{
            'shapeType':'convexTriangle'
        });
        //포탈!!(맵간 이동) 별도 파라미터 넣지않으면 이동시 맵의 초기설정위치에서 로드,pos넣을시 그 post에 관련된 위치에서 카메라랑,캐릭터랑 해서 로드
        this.NextPORTAL('https://sinjaesung.github.io/3DASSET/terrain/evil/evilFactory_Nextfortal.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{
            'shapeType':'convexTriangle'
        });
        this.StartPORTAL('https://sinjaesung.github.io/3DASSET/terrain/evil/evilFactory_Startfortal.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,{
            'shapeType':'convexTriangle'
        });  
        ...
        buildlingColor.setRGB(0.7,0.5,0.2); 
        ...
        for(let a=0; a<2; a++){
            ...
            audioLoader_buildingTest.load('https://sinjaesung.github.io/3DASSET/mob/normalBuildingCollapse.mp3',function(buffer){//무기공격성공시사운드 다르게
                ...
                collapseSound.setVolume(4);
            });
            collapseSounds.push(collapseSound);
        }
        this.createGLTF_EnemyBuildinggroup('https://sinjaesung.github.io/3DASSET/terrain/evil/evilfactory_enemyFactoryCoreGroup.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,40000,100,buildlingColor,collapseSounds)

        buildlingColor=new THREE.Color();
        buildlingColor.setRGB(0.5,0.8,0.4); 
        collapseSounds=[];
        for(let a=0; a<2; a++){
            let audioListener_buildlingTest=new THREE.AudioListener();
            let collapseSound=new THREE.Audio(audioListener_buildlingTest);
        ...
        this.createGLTF_EnemyBuildinggroup('https://sinjaesung.github.io/3DASSET/terrain/evil/evilfactory_enemyFactory1Group.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,26000,80,buildlingColor,collapseSounds)

        buildlingColor=new THREE.Color();
        buildlingColor.setRGB(0.9,0.3,0.4); 
        collapseSounds=[];
        for(let a=0; a<2; a++){
            let audioListener_buildlingTest=new THREE.AudioListener();
            let collapseSound=new THREE.Audio(audioListener_buildlingTest);
        ...
        console.log("collapseSound뭔데?:",collapseSounds)
        this.createGLTF_EnemyBuildinggroup('https://sinjaesung.github.io/3DASSET/terrain/evil/evilfactory_enemyFactory2Group.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,20000,60,buildlingColor,collapseSounds)
        ...
        console.log("collapseSound뭔데?:",collapseSounds)
        this.createGLTF_EnemyBuildinggroup('https://sinjaesung.github.io/3DASSET/terrain/evil/evilfactory_enemyFactory3Group.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,16000,60,buildlingColor,collapseSounds)

        buildlingColor=new THREE.Color();
        buildlingColor.setRGB(0.99,0.1,0.2); 
        collapseSounds=[];
        for(let a=0; a<2; a++){
            let audioListener_buildlingTest=new THREE.AudioListener();
            let collapseSound=new THREE.Audio(audioListener_buildlingTest);
        ...
        console.log("collapseSound뭔데?:",collapseSounds)
        this.createGLTF_EnemyBuildinggroup('https://sinjaesung.github.io/3DASSET/terrain/evil/evilfactory_enemyFactory4Group.glb',Ammo,null,{x:0,y:0,z:0,w:1},0,10000,40,buildlingColor,collapseSounds)

    
        // 몬스터
        for(let e=0; e<6; e++){//humanBad created Types (메카닉:전투기계들,바이오닉,사이보그) 상급boss
            let unit=e%2==0?-1:1;
         ...
         for(let e=0; e<10; e++){//humanBad created Types (메카닉:전투기계들,바이오닉,사이보그)
            ...
            let mass=20;
            let defense=200;
            let distanceamount=100;
            let isAir=true;
            let isAirChase=true;

            let attackRangeAmount=10;
            ...
            this.createEnemyGLTF3(Ammo,pos,"https://sinjaesung.github.io/3DASSET/mob/FarCreatedMonster(redAir).glb",null,e,hp,power,'farType',speed,mass,'mecha1Air','https://sinjaesung.github.io/3DASSET/mob/FarCreatedMonster(redAir)_ballchild.glb',skillPos,null,defense,distanceamount,isAir,attackRangeAmount,attackParticlesrc,attackSound,attackColor,isAirChase,2,1,diedSound);
        }
       for(let e=0; e<10; e++){//humanBad created Types (메카닉:전투기계들,바이오닉,사이보그)
            let unit=e%2==0?-1:1;
            let x=Math.random()*80*unit; //-500~500범위가능
            ...
            let mass=400;
            let defense=400;
            let distanceamount=40;
            let isAir=false;
            let isAirChase=true;

            let attackRangeAmount=8;
            let attackParticlesrc='https://sinjaesung.github.io/3DASSET/mob/fire2.jpg';
            let attackColor=new THREE.Color();
            attackColor.setRGB(0,0.4,0.9); 

            let audioListener_individual=new THREE.AudioListener();
            let attackSound=new THREE.Audio(audioListener_individual);
            let audioLoader_local=new THREE.AudioLoader();
            audioLoader_local.load('https://sinjaesung.github.io/3DASSET/mob/smallbomb.mp3',function(buffer){//무기공격성공시사운드 다르게
                attackSound.setBuffer(buffer);
                attackSound.setLoop(false);
                attackSound.setVolume(0.3);
            });
            ...
            this.createEnemyGLTF3(Ammo,pos,"https://sinjaesung.github.io/3DASSET/mob/FarCreatedMonster2(blueNoair).glb",null,e,hp,power,'farType',speed,mass,'mecha1ground','https://sinjaesung.github.io/3DASSET/mob/FarCreatedMonster2(blueNoair)_ballchild.glb',skillPos,null,defense,distanceamount,isAir,attackRangeAmount,attackParticlesrc,attackSound,attackColor,isAirChase,2,1,diedSound);
        }
        
        let added_loadmodel_bg={};
        added_loadmodel_bg['src']="https://sinjaesung.github.io/3DASSET/goegap_4k.hdr";
        added_loadmodel_bg['isloaded']=false;
        this.loadmodels.push(added_loadmodel_bg);

        new RGBELoader().load("https://sinjaesung.github.io/3DASSET/goegap_4k.hdr",(texture)=>{
